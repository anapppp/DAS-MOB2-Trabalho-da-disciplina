...
class CharactersInAHouseActivity : AppCompatActivity() {
    ...
    private lateinit var tvCharList: TextView
    ...
    private fun fetchStudentsByHouse(house: String) {
        lifecycleScope.launch {
            try {
                showLoading(progressBar, tvCharList)
                val staff: List<Character>? = viewModel.getCharactersByHouse(house)
                val names = staff?.joinToString(separator = "\n") { it.name ?: "" }

                tvCharList.text = names


            } catch (e: Exception) {
                Toast.makeText(
                    this@CharactersInAHouseActivity,
                    "Erro ao buscar professores",
                    Toast.LENGTH_SHORT
                ).show()
                Log.e("HogwartsStaffActivity", "Erro ao buscar professores", e)
            } finally {
                hideLoading(progressBar, tvCharList)
            }
        }
    }
}
